...
class WorkerUITests(TestCase):
    def setUp(self):
        ...
        self.operation = Operations.objects.create(
            id=1, name="Test Operation", price=100
        )

    def test_pay_salary(self):
        url = reverse("pay_salary_view", kwargs={"worker_id": self.worker.id})
        print(f"Testing URL: {url}")  # Отладочная информация
        response = self.client.post(url, data={"amount": "1000.00"})
    ...
    def test_process_operation(self):
        url = reverse(
            ...
            kwargs={"worker_id": self.worker.id, "operation_id": self.operation.id},
        )
        print(f"Testing URL: {url}")  # Отладочная информация
